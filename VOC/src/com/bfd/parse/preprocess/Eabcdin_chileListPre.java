 ...
 *
 */
public class Eabcdin_chileListPre implements PreProcessor {
	private static final Log LOG = LogFactory.getLog(Eabcdin_chileListPre.class);
	...
	public String parsePageData(String pageData) {
		//删除价格中Internet字段
		pageData = pageData.replaceAll("Internet<br class=\"showonMobile\">&nbsp;", "").trim();
		//处理title显示不全
		HtmlCleaner cleaner = new HtmlCleaner();
		TagNode root = cleaner.clean(pageData);
		try {
			Object[] objs = root.evaluateXPath("//div[@class='image']/a");
			for (Object obj : objs) {
