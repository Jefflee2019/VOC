...
import com.bfd.parse.reprocess.NqqvideoListRe;
import com.bfd.parse.util.JsonUtil;

public class NqqvideoListReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "https://v.qq.com/x/search/?q=华为&stag=102&smartbox_ab=";
		// 动态数据
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nqqvideo_list.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new NqqvideoListRe(), tmplStr, face, outputFiled);
