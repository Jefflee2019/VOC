...
import com.bfd.parse.reprocess.Npc841ListRe;
import com.bfd.parse.util.JsonUtil;

public class Npc841ListReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url = "http://zhannei.baidu.com/cse/search?q=%E5%8D%8E%E4%B8%BA&s=4929329715495988808&srt=lds&nsid=0&wt=2&ht=2&pn=20";
		// 动态数据
...
//		ajaxlist.add(null);
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Npc841_list.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new Npc841ListRe(), tmplStr, face,
				outputFiled);
