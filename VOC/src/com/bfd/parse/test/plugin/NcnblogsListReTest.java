...
import com.bfd.parse.reprocess.NcnblogsListRe;
import com.bfd.parse.util.JsonUtil;

public class NcnblogsListReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url = "http://zzk.cnblogs.com/s/news?Keywords=华为";
		// 动态数据
...
//		ajaxlist.add(null);
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Ncnblogs_list.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new NcnblogsListRe(), tmplStr, face,
				outputFiled);
