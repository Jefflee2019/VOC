...
import com.bfd.parse.reprocess.BzolListRe;
import com.bfd.parse.reprocess.EamazonCommentRe;
import com.bfd.parse.util.JsonUtil;

public class BzolListReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "http://ask.zol.com.cn/cell_phone/apple/";
		// 动态数据
...
//		 ajaxlist.add(url1);
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/BzolListRe.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new BzolListRe(), tmplStr, face, outputFiled);
