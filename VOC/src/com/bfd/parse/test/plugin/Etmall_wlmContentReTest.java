...
import com.bfd.parse.reprocess.Etmall_wlmContentRe;
import com.bfd.parse.util.JsonUtil;

public class Etmall_wlmContentReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "https://detail.tmall.com/item.htm?id=557806596358&skuId=3464849753559";
		// 动态数据
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Etmall_wlm_con_10.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new Etmall_wlmContentRe(), tmplStr, face, outputFiled);
