...
import com.bfd.parse.json.NdonewsListJson;
import com.bfd.parse.util.JsonUtil;

public class NdonewsListTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url = "http://www.donews.com/search/more_search_news?keyword=华为&page=1";
		// 动态数据
		List<String> ajaxlist = new ArrayList<String>();
		ajaxlist.add("http://www.donews.com/search/more_search_news?keyword=华为&page=1");
		// 生成解析使用的bean
		...
		// 模板
//		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nyangtse_list.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, new NdonewsListJson(), null, null, face,
				outputFiled);
