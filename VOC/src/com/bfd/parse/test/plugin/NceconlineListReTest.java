...
import com.bfd.parse.reprocess.NceconlineListRe;
import com.bfd.parse.util.JsonUtil;

public class NceconlineListReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "http://www.ceconline.com/FSSearch.do?keyword=%BB%AA%CE%AA&pageno=01&sortBy=0";
		// 动态数据
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nceconline_list_1.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new NceconlineListRe(), tmplStr, face, outputFiled);
