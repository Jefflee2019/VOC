...
public class BnewsmthPostReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "http://www.newsmth.net/nForum/article/Xiaomi/175928";
		// 动态数据
...
//		 ajaxlist.add(url1);
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/BnewsmthPostRe.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new BnewsmthPostRe(), tmplStr, face, outputFiled);
