...
import com.bfd.parse.reprocess.NlexunContentRe;
import com.bfd.parse.util.JsonUtil;

public class NlexunContentTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url = "http://sjnews.lexun.cn/touch/detail.aspx?Textid=173773";
		// 动态数据
...
//		ajaxlist.add(null);
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nlexun_content_1.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new NlexunContentRe(), tmplStr, face,
				outputFiled);
