...
import com.bfd.parse.reprocess.BrightPostRe;
import com.bfd.parse.util.JsonUtil;

public class BrightPostReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "http://www.right.com.cn/forum/forum.php?mod=viewthread&tid=193350&extra=&highlight=%BB%AA%CE%AA&page=2";
		// 动态数据
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Bright_post_1.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new BrightPostRe(), tmplStr, face, outputFiled);
