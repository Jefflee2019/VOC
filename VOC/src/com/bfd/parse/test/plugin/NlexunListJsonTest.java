...
import com.bfd.parse.json.NlexunListJson;
import com.bfd.parse.util.JsonUtil;

public class NlexunListJsonTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url = "http://sjnews.lexun.cn/touch/list.aspx?title=华为&search=1&p=1";
		// 动态数据
		List<String> ajaxlist = new ArrayList<String>();
		ajaxlist.add("http://sjnews.lexun.cn/touch/list.aspx?title=华为&search=1&p=1");
		// 生成解析使用的bean
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nlexun_list.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, new NlexunListJson(), null, tmplStr, face,
				outputFiled);
