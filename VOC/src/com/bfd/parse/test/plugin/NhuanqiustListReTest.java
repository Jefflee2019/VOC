...
import com.bfd.parse.reprocess.NhuanqiustListRe;
import com.bfd.parse.util.JsonUtil;

public class NhuanqiustListReTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url1 = "http://news.baidu.com/ns?word=%E5%8D%8E%E4%B8%BA+site%3Asports.huanqiu.com&pn=10&ct=0";
		// 动态数据
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nhuanqiust_list_1.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, null, new NhuanqiustListRe(), tmplStr, face, outputFiled);
