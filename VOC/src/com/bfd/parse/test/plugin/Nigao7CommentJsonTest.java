...
import com.bfd.parse.json.Nigao7CommentJson;
import com.bfd.parse.util.JsonUtil;

public class Nigao7CommentJsonTest {
	...
	public static void testDemo() throws Exception {
		// 要抓取链接
		String url = "http://home.igao7.com/comment/loadCommentJson?item_id=121297&type=2";
		// 动态数据
		List<String> ajaxlist = new ArrayList<String>();
		ajaxlist.add("http://home.igao7.com/comment/loadCommentJson?item_id=121297&type=2");
		// 生成解析使用的bean
		...
		// 模板
		String tmplStr = FileUtils.readFileToString(new File("tmpl/Nigao7_comment.txt"));
		// 解析输出字段
		String outputFiled = "";
		ParserFace face = new ParserFace("test");
		// 执行解析流程
		// 参数分别是 预处理插件,json插件,后处理插件,模板,ParseFace,输出字段
		ParsePlugin testPlugin = new ParsePlugin(null, new Nigao7CommentJson(), null, tmplStr, face,
				outputFiled);
